...
import 'package:neurotask_ng/services/color_game_services.dart';
import 'package:neurotask_ng/ui/game/game_start_message.dart';
...
class _ColorGameState extends State<ColorGame> {
    ...
    builder: (context) {
      return AlertDialog(
        ...
        content: const Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start, // Optional: align text to left
          children: [
            MyText(
              text: "Speak the color you see, not the ",
              size: 12,
              bold: false,
              color: Colors.black,
              height: 0.05,
            ...
            MyText(
              text: "word you read. Say it out loud!",
              size: 8,
              bold: false,
              color: Colors.black,
              height: 0.05,
            ...
            children: [
              TextButton(
                ...
                child: const MyText(
                  ...
                  size: 20, 
                  bold: true, 
                  color: Color.fromARGB(255, 198, 198, 4),
                  height: 0.05,
                  width: 0.05,
                ),
              ),
  ...
  int remainingTime = 30; // total time for the game in seconds
  String formattedTime = "30s";
  ...
  countdownTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
    if (remainingTime <= 0) {
      ...
      // Optionally show game-over screen or message here
    } else {
      setState(() {
        remainingTime--;
      ...
      if (remainingTime % intervalTime == 0) {
        ...
        if (index < colorMap.length) {
          index++;
          stopSpeechRecord();
          ColorGameServices.colorGameDataToFirebase(
            deviceTime!,
            disapearTime!,
            (positionX * screenWidth),
            (positionY * screenHeight),
            colorMap.keys.elementAt(index - 1),
            textColors[index - 1],
            voiceToText,
            intervalTime,
          );
                                ...
                                Row(
                                  children: [
                                    Text(
                                      formattedTime,
                                      style: TextStyle(
                                        fontSize: (width / Responsive.designWidth) * 25,
